...
// 클래스 외부에서 GET 요청 처리
async function sendGetRequest(url = "http://localhost:8000/temp/update") {
    try {
  ...
  async function getFileList() {
      try {
          // 서버에서 파일 리스트 받아오기
          const fileList = await sendGetRequest("http://localhost:8000/temp/list");
